...
def emit_new_user_to_others(socketio, sid, pin):
    ...
    for i in users:
        socketio.emit(
            "new_user_joined",
            {"username": username, "teamname": team_name},
            to=i["sid"],
        )
...
def emit_new_team_name_to_others(socketio, new_team_name, old_team_name, pin):
    ...
    for i in users:
        socketio.emit(
            "new_team_name",
            {"teamname": new_team_name, "oldteamname": old_team_name},
            to=i["sid"],
        )
...
def emit_start_round_to_others(
    socketio, pin, sid, word, is_last_word, time_at_start, start_date
):
    users = userdata.get_users_from_lobby(pin)

    print(f"emit_start_round_to_others: {users}")
    print(f"emit_start_round_to_others: {sid}")

    for i in users:
        if i["sid"] != sid:
            socketio.emit(
                "start_round",
                {"timeatstart": time_at_start, "startdate": start_date},
                to=i["sid"],
            )
        else:
            socketio.emit(
                ...
                {
                    "word": word,
                    "islastword": is_last_word,
                    "timeatstart": time_at_start,
